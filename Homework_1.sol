...
contract Example {
    ...
    // We will use this modifier with our execute functions.
    // This modifier makes sure that the caller of the function is the owner of the contract.
    modifier onlyOwner() {
    ...
    // They also have the onlyOwner modifier which we created above. This makes sure that
    ...
    // This function is similar to the one above, but instead of increasing, we decrease the number by one.
    function decrement_counter() external onlyOwner {
        counter.number -= 1;
    }

    // The function below is a query function.
    // It does not change any data on the chain. It just retrieves data.
    ...
    // This function is a query function that returns the description field of our counter struct.
    // It retrieves the description without modifying any data on the chain.
